...
def crop_grid_image(grid_image: Image.Image, grid_size: Tuple[int, int] = (2, 2)) -> Dict[str, Image.Image]:
    """
    Crop a grid image into individual sections.
    
    Args:
        grid_image: The grid image to crop.
        grid_size: Tuple of (rows, cols) for the grid. Default is (2, 2) for a 2x2 grid.
    
    Returns:
        Dictionary with keys: "front", "side", "back", "three_quarter"
        Each value is a PIL Image of the cropped view.
    """
    width, height = grid_image.size
    
    # Detect actual grid layout by checking if it looks like 3x3 instead of 2x2
    # If image is 2048x2048, a 2x2 grid would have cells of ~1024x1024
    # A 3x3 grid would have cells of ~682x682
    # Check by looking at aspect ratio and size
    expected_cell_size_2x2 = width // 2
    expected_cell_size_3x3 = width // 3
    
    # Heuristic: if the image dimensions suggest 3x3 (divisible by 3), warn and use 3x3
    # But we still want to extract 4 views from a 3x3 grid
    # For 3x3, we'll take the corner cells for maximum view separation
    if width % 3 == 0 and height % 3 == 0 and grid_size == (2, 2):
        logger.warning(f"Grid image {width}x{height} appears to be 3x3 instead of 2x2. Attempting to extract 4 views from 3x3 grid corners.")
        # Extract from 3x3 corners: use positions (0,0), (0,2), (2,0), (2,2)
        # which correspond to top-left, top-right, bottom-left, bottom-right
        cell_width = width // 3
        cell_height = height // 3
        
        cropped_images = {}
        # Map 3x3 corner positions to our 4 views
        # Top-left (0,0) -> front, Top-right (0,2) -> side
        # Bottom-left (2,0) -> back, Bottom-right (2,2) -> three_quarter
        positions_map = [
            ((0, 0), "front"),      # Top-left
            ((0, 2), "side"),      # Top-right
            ((2, 0), "back"),       # Bottom-left
            ((2, 2), "three_quarter")  # Bottom-right
        ]
        
        for (row, col), position in positions_map:
            left = col * cell_width
            top = row * cell_height
        ...
        logger.info(f"Extracted 4 views from 3x3 grid corners: {list(cropped_images.keys())}")
        return cropped_images
    
    # Normal 2x2 grid processing
